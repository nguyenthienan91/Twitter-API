...
class UsersService {
  ...
  private signEmailVerifyToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    return signToken({
      ...
      options: {
        expiresIn: (process.env.EMAIL_VERIFY_TOKEN_EXPIRES_IN || '7d') as any
      }
    })
  }
  private signForgotPasswordToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    return signToken({
      ...
      options: {
        expiresIn: (process.env.FORGOT_PASSWORD_TOKEN_EXPIRES_IN || '7d') as any
      }
    })
  }
  ...
  async oauth(code: string) {
    const { id_token, access_token } = await this.getOauthGoogleTokens(code)
    ...
    if (user) {
      ...
      // không thì đăng ký tài khoản mới
  ...
  async forgotPassword({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    ...
    // Gửi email đến cho người dùng https://twitter.com/forgot-password?token=forgot_password_token
    console.log('Forgot password token: ', forgot_password_token)
    return {
      message: USERS_MESSAGES.PASSWORD_HAS_BEEN_RESET
    }
  }
  ...
  async follow(user_id: string, followed_user_id: string) {
    ...
    return {
      message: USERS_MESSAGES.YOU_HAVE_FOLLOWED_THIS_ACCOUNT
    }
  }
  ...
  async unFollow(user_id: string, followed_user_id: string) {
    ...
    return {
      message: USERS_MESSAGES.YOU_HAVE_NOT_FOLLOWED_THIS_ACCOUNT
    }
  }
