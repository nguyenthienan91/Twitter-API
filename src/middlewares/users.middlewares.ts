...
const confirmPasswordSchema: ParamSchema = {
  notEmpty: {
    errorMessage: USERS_MESSAGES.CONFIRMED_PASSWORD_IS_REQUIRED
  },
  isString: {
    errorMessage: USERS_MESSAGES.CONFIRMED_PASSWORD_MUST_BE_A_STRING
  },
  isLength: {
    options: {
      min: 6,
    ...
    errorMessage: USERS_MESSAGES.CONFIRMED_PASSWORD_LENGTH_MUST_BE_FROM_6_TO_50
  },
  isStrongPassword: {
    options: {
      minLength: 6,
    ...
    errorMessage: USERS_MESSAGES.CONFIRMED_PASSWORD_MUST_BE_STRONG
  },
  custom: {
    options: (value, { req }) => {
      if (value !== req.body.password) {
        throw new Error(USERS_MESSAGES.CONFIRMED_PASSWORD_MUST_MATCH)
      }
      return true
    }
  }
  ...
  custom: {
    options: async (value: string, { req }) => {
      if (!ObjectId.isValid(value)) {
        throw new ErrorWithStatus({
          message: USERS_MESSAGES.INVALID_USER_ID_FOLLOWER,
...
export const validPasswordValidator = validate(
  checkSchema(
    {
      email: {
        isEmail: {
